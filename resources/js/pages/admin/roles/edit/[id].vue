<template>
  <AdminLayout>
    ...
    <div class="max-w-5xl">
      <h1 class="text-3xl font-bold mb-6" :style="{ color: 'var(--ink)' }">{{ $t('edit_role') }}</h1>
      <div v-if="role" class="rounded-lg p-6" :style="{ background: 'var(--surface)', border: '1px solid var(--border)' }">
...
const breadcrumbs = [{ label: 'Roles', labelBn: 'ভূমিকা' }, { label: 'Edit', labelBn: 'সম্পাদনা' }]
...
onMounted(async () => {
  ...
  try {
    const [perms, r] = await Promise.all([
      ...
      admin.fetchRole(route.params.id),
    ])
    allPermissions.value = perms || []
    role.value = r
    form.value = { name: r.name, label: r.label || '', permissions: (r.permissions || []).map(p => p.id) }
  } catch (e) { router.push('/admin/roles') }
})

async function saveRole() {
  try {
    await admin.updateRole(route.params.id, form.value)
